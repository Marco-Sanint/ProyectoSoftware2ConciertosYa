...
import Header from "./Header";
    ...
    setSeats((prev) =>
        prev.map((s) => (s.id === seatId ? { ...s, selected: true } : s))
    );
    setCart((prev) => [...prev, seat]);
    }
};

// Maneja la devolución de un asiento
const handleReturn = (seatId) => {
    const seat = cart.find((s) => s.id === seatId);
    if (seat) {
    setCart((prev) => prev.filter((s) => s.id !== seatId)); // Eliminar del carrito
    setSeats((prev) =>
        prev.map((s) => (s.id === seatId ? { ...s, selected: false } : s))
